"""
GaussianImageDistribution - D2P (B6) Evaluation Script

Evaluates D2P using image-text retrieval R@K.

Usage:
    python scripts/evaluate_d2p.py
...
logger = get_logger("eval_d2p", config.EVAL_D2P_LOG_PATH)


def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate D2P (B6)")
...
def main():
    ...
    checkpoint_path = args.checkpoint or str(config.D2P_BEST_CKPT)
    logger.info(f"Loading D2P from {checkpoint_path}")
    ...
    output_path = args.output_path or str(config.D2P_EVAL_RESULTS_PATH)
...
if __name__ == "__main__":
    try:
        ...
        log_exception(logger, e, "D2P evaluation failed")
        raise
