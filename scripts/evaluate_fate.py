"""
GaussianImageDistribution - FATE Evaluation Script

This script evaluates the FATE model using image-text retrieval
with Recall@K metrics. Uses a random subset of samples for efficiency.

Usage:
    python scripts/evaluate_fate.py
    python main.py --task eval_fate
"""
...
logger = get_logger("eval_fate", config.EVAL_FATE_LOG_PATH)


def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(description="Evaluate FATE Model")
...
def extract_features(
    ...
    """Extract image features and adapted text features."""
    model.eval()
    ...
    for batch in tqdm(dataloader):
        ...
        # Extract adapted text features (with vision perturbation)
        text_feat = model.encode_text_adapted(input_ids, attention_mask, img_feat)
...
def main():
    ...
    # Load model
    checkpoint_path = args.checkpoint or str(config.FATE_BEST_CKPT)
    ...
    # Save results
    output_path = args.output_path or str(config.FATE_EVAL_RESULTS_PATH)
    results = {
