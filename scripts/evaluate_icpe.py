"""
GaussianImageDistribution - ICPE (B5) Evaluation Script

ICPE is training-free: uses frozen CLIP features with k-NN covariance.
Evaluates using image-text retrieval R@K.

Usage:
    python scripts/evaluate_icpe.py
...
logger = get_logger("eval_icpe", config.EVAL_ICPE_LOG_PATH)


def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate ICPE (B5)")
    parser.add_argument("--checkpoint", type=str, default=None,
                        help="ICPE config path (optional, training-free)")
...
def extract_features(model, dataloader, device, num_samples=None):
    """Extract CLIP features (ICPE uses raw CLIP features as mu)."""
    model.eval()
    all_img, all_text = [], []
    sample_count = 0
    ...
    for batch in tqdm(dataloader, desc="Extracting features"):
        ...
        # ICPE forward: returns img_mu=text_mu=CLIP features (no transformation)
...
def main():
    ...
    logger.info("ICPE (B5) - Training-free evaluation")
    ...
    output_path = args.output_path or str(config.ICPE_EVAL_RESULTS_PATH)
    with open(output_path, 'w') as f:
        json.dump({'model_type': 'icpe', 'num_samples': num_samples, 'metrics': metrics}, f, indent=2)
...
if __name__ == "__main__":
    try:
        ...
        log_exception(logger, e, "ICPE evaluation failed")
        raise
